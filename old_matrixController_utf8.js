...
export const processMatrixPlacement = async (userId, parentId) => {
    try {
        ...
        if (!parent) {
            console.log('Γ¥î Parent not found');
            return;
        }
        ...
        if (parent.matrix.level1.filled < parent.matrix.level1.total) {
            ...
            console.log('Γ₧í∩╕Å  Placing in Level 1, Position:', position);
            ...
            await parent.save();
            console.log('Γ£à Placed in Level 1. New filled count:', parent.matrix.level1.filled);

            // Check if Level 1 completed
            if (parent.matrix.level1.filled === parent.matrix.level1.total) {
                console.log('≡ƒÄë LEVEL 1 COMPLETED!');
                ...
                if (parent.matrix.cycle > 5) {
                    console.log(`ΓÜá∩╕Å User ${parent.fullname} has completed 5 cycles. Deactivating for reactivation.`);
                    parent.verified = false;
                    parent.paymentStatus = 'pending';
                }

                await parent.save();
                console.log('≡ƒöä Matrix reset for new cycle:', parent.matrix.cycle);
            }
        } else {
            console.log('ΓÜá∩╕Å  Matrix full! Level 1 completed.');
        }

        console.log('=== MATRIX PLACEMENT COMPLETE ===');
    } catch (error) {
        console.error('Γ¥î Matrix placement error:', error);
...
const creditMatrixIncome = async (userId, level) => {
    try {
        ...
        if (user.matrix.cycle > 5) {
            console.log(`ΓÅ╣∩╕Å Cycle limit reached (${user.matrix.cycle}). No matrix income credited.`);
            return;
        }

        const incomeAmount = 1200; // Member Income: Γé╣1,200
        console.log(`≡ƒÆ░ Crediting Matrix Level ${level} Income (Cycle ${user.matrix.cycle}) to Matrix Wallet: Γé╣${incomeAmount}`);
        ...
        if (user.referredBy) {
            ...
            if (referrer) {
                ...
                console.log(`≡ƒææ Crediting Leadership Referral Royalty to ${referrer.fullname}: Γé╣${royaltyAmount}`);
        ...
        console.error('Γ¥î Credit matrix income error:', error);
    }
};
